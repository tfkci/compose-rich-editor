...
internal object RichTextStateHtmlParser : RichTextStateParser<String> {
    ...
    override fun decode(richTextState: RichTextState): String {
        ...
        paragraphs.fastForEachIndexed { index, richParagraph ->
            ...
            val nextImmediateIsContinuation = nextParagraph != null &&
                nextParagraph.isFromLineBreak &&
                !nextParagraph.isEmpty()

            // For flow text, every empty paragraph in between is emitted as a bare
            // <br> INSIDE the still-open tag, so the continuation check must look
            // past ALL of them (whether or not they are flagged isFromLineBreak —
            // an explicit "</p><br>" leaves an empty non-line-break paragraph).
            // Closing the tag too early pushes the inline <br> content after </p>,
            // producing malformed, non-idempotent HTML (e.g.
            // "<p>First</p><br><br>Second</p>") whose re-parse accumulates an extra
            // <br> on every save/reload cycle. List items keep the
            // immediate-next check so list grouping is untouched.
            val nextIsContinuationPastEmpties: Boolean = run {
                var lookaheadIndex = index + 1
                var result = false
                while (true) {
                    val candidate = paragraphs.getOrNull(lookaheadIndex) ?: break
            ...
            if (richParagraph.isFromLineBreak && openItemTag != null) {
                ...
                val continuationKeepsTagOpen =
                    if (openItemTag == "li") nextImmediateIsContinuation
                    else nextIsContinuationPastEmpties
                if (!continuationKeepsTagOpen) {
            ...
            if (isParagraphList) {
                ...
                while (openLists.size < paragraphLevel) {
                    ...
                    if (
                        ...
                        paragraphGroupTagName == "ul" &&
                        richParagraphType is UnorderedList &&
                        richParagraphType.styleType != DefaultUnorderedListStyleType &&
                        openingLevel == paragraphLevel
                    ) {
                        // Persist non-default bullet styles across save/reload
            ...
            if (isParagraphEmpty && !isParagraphList) {
                ...
                // Keep this tag open when a following non-empty <br> continuation
                // paragraph will emit its content inline relying on it (see the
                // past-empties lookahead computed above for flow text).
                val nextIsLineBreakContinuation =
                    if (paragraphTagName == "li") nextImmediateIsContinuation
                    else nextIsContinuationPastEmpties
